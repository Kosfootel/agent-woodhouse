...
const AlertsPage = () => {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:8005'}/api/alerts`);
        if (response.ok) {
  ...
  const acknowledgeAlert = async (alertId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL || 'http://localhost:8005'}/api/alerts/${alertId}/acknowledge`,
        { method: 'POST' }
      );
      if (response.ok) {
